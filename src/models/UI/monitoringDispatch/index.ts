  ...
  effects: {
    ...
    fetchMediaServiceEngine(config): void {
      ...
      const {
        ...
        onLogout,
        onTempGroupList,
        onCallingStartResponse,
        onCallingStop,
        onCreateTempGroupResponse,
        onTempGroupUpdate,
        onAddTempGroupMemberResponse,
        updateState,
        login,
        onDuplexCallingRing
    ...
    onLogout(): void {
      store.dispatch.monitoringDispatch.updateState({loginResponseStatus: false, config: undefined});
    ...
    remoteControlMs(request: RemoteControlMsRequest): void {
      ...
      if (hjMediaEngine) {
        ...
        store.dispatch.log.addLog({
          type: request.controlCmd === ControlCmd.BAN ? LogType.Ban : LogType.LiftBan,
